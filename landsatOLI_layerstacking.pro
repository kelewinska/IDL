...
;+
; :Description: script layerstack Landsat OLI bands and masks claouds and clouds shadows based on QA band
;
; :Outputs: LC08_L1TP_PPPRRR_YYYMMDD_layerstackM.bsq  - masked layerstack of bands
;           LC08_L1TP_PPPRRR_YYYMMDD_DataMask.bsq     - binary mask derived based on QA
;
...
PRO landsatOLI_layerstacking
  ...
  FOR j=0L, last-1 DO BEGIN
    ...
    bit8 = bitget(data_vector[j], 8, SILENT=silent)
    bit9 = bitget(data_vector[j], 9, SILENT=silent)

    IF (bit5[0] EQ 1) AND ((bit6[0] EQ 1) OR (bit6[0] EQ 1)) OR (bit3[0] EQ 1) OR (bit4[0] EQ 1) OR (bit8[0] EQ 1 AND bit9[0] EQ 1) THEN bitdataclass[j]=0 ELSE bitdataclass[j]=1
